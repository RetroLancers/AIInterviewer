...
cd AIInterviewer.Client && npm run dtos
```

**Tailwind CSS (for Razor Pages):**
```bash
# Watch mode for styling (from AIInterviewer directory)
npm run ui:dev
...
```bash
cd AIInterviewer && dotnet watch
```

**Using tailwind in new Razor Pages:**

Terminal 2: `cd AIInterviewer && npm run ui:dev` (Tailwind watch)
...
```bash
# Run all .NET tests
cd AIInterviewer.Tests && dotnet test
```
...
cd AIInterviewer && npm run migrate
...
cd AIInterviewer && npm run revert:last

# Drop and re-run last migration (useful during development)
cd AIInterviewer && npm run rerun:last
```
...
**Development Mode:**
- `dotnet watch` from AIInterviewer starts .NET (port 5001) and Vite dev server (port 5173), accessible via `https://127.0.0.1:5173`
- ASP.NET Core proxies requests to Vite dev server via `NodeProxy` (configured in [Program.cs](AIInterviewer/Program.cs#L41))
- Hot Module Replacement (HMR) enabled via WebSocket proxying using `MapNotFoundToNode`, `MapViteHmr`, `RunNodeProcess`, `MapFallbackToNode` in [Program.cs](AIInterviewer/Program.cs)

**Production Mode:**
- Vite builds Vue app to `AIInterviewer.Client/dist/`, which is copied to `AIInterviewer/wwwroot/` when published
- ASP.NET Core serves static files directly from `wwwroot` - no Node.js required
- Fallback to `index.html` for client-side routing

### Modular Startup Configuration

AppHost uses .NET's `IHostingStartup` pattern to split configuration across multiple files in `AIInterviewer/`:
- [Configure.AppHost.cs](AIInterviewer/Configure.AppHost.cs) - Main ServiceStack AppHost registration
- [Configure.Auth.cs](AIInterviewer/Configure.Auth.cs) - ServiceStack AuthFeature with ASP.NET Core Identity integration
- [Configure.AutoQuery.cs](AIInterviewer/Configure.AutoQuery.cs) - AutoQuery features and audit events
- [Configure.Db.cs](AIInterviewer/Configure.Db.cs) - Database setup (OrmLite for app data, EF Core for Identity)
- [Configure.Db.Migrations.cs](AIInterviewer/Configure.Db.Migrations.cs) - Runs OrmLite and EF DB Migrations and creates initial users
- [Configure.BackgroundJobs.cs](AIInterviewer/Configure.BackgroundJobs.cs) - Background job processing
- [Configure.HealthChecks.cs](AIInterviewer/Configure.HealthChecks.cs) - Health monitoring endpoint

This pattern keeps [Program.cs](AIInterviewer/Program.cs) clean and separates concerns. Each `Configure.*.cs` file is auto-registered via `[assembly: HostingStartup]` attribute.

### Project Structure

```
AIInterviewer/                         # .NET Backend (hosts both .NET and Vite build output)
...
AIInterviewer.ServiceModel/            # DTOs & API contracts
...
AIInterviewer.Tests/                   # .NET tests (NUnit)
...
- Configuration in [vite.config.markdown.ts](AIInterviewer.Client/vite.config.markdown.ts)
...
Reference: Browse [src/pages/](AIInterviewer.Client/src/pages/) for usage examples.
...
**Migration Files:**
- `AIInterviewer/Migrations/20240301000000_CreateIdentitySchema.cs` - EF Core migration for Identity
- `AIInterviewer/Migrations/Migration1000.cs` - OrmLite migration for app tables (e.g., Booking)
...
2. ServiceStack AuthFeature integrates with Identity via `IdentityAuth.For<ApplicationUser>()` in [Configure.Auth.cs](AIInterviewer/Configure.Auth.cs)
3. Custom claims added via `AdditionalUserClaimsPrincipalFactory` and `CustomUserSession`
4. ServiceStack services use `[ValidateIsAuthenticated]` and `[ValidateHasRole]` attributes for authorization (see [Bookings.cs](AIInterviewer.ServiceModel/Bookings.cs))

### ServiceStack .NET APIs

ServiceStack APIs adopt a [DTOs-first approach utilizing message-based APIs](https://docs.servicestack.net/api-design). To create ServiceStack APIs create all related DTOs used in the API (aka Service Contracts) into a single file in the `AIInterviewer.ServiceModel` project, e.g:

```csharp
//AIInterviewer.ServiceModel/Bookings.cs
...
ServiceStack API implementations should be added to `AIInterviewer.ServiceInterface/`:

```csharp
//AIInterviewer.ServiceInterface/BookingServices.cs
...
ServiceStack's AutoQuery generates full CRUD APIs from declarative request DTOs. Example in [Bookings.cs](AIInterviewer.ServiceModel/Bookings.cs):
...
After changing C# DTOs in `AIInterviewer.ServiceModel/`, restart the .NET Server then run:
```bash
cd AIInterviewer.Client && npm run dtos
```
...
1. **TypeScript data model** (`AIInterviewer.ServiceModel/Bookings.d.ts`) defines the entity with decorators
2. **C# AutoQuery APIs** (`AIInterviewer.ServiceModel/Bookings.cs`) - auto-generated CRUD request/response DTOs
3. **C# OrmLite migration** (`AIInterviewer/Migrations/Migration1000.cs`) - auto-generated schema creation

This enables rapid prototyping: edit the `.d.ts` model, run `npx okai Bookings.d.ts`, then `npm run migrate`.

**Important:** The `.d.ts` files use special decorators (e.g., `@validateHasRole`, `@autoIncrement`) that map to C# attributes and .NET Types. The valid schema for these is defined in [api.d.ts](AIInterviewer.ServiceModel/api.d.ts). Reference [Bookings.d.ts](AIInterviewer.ServiceModel/Bookings.d.ts) for examples.
...
The template includes Razor Pages for Identity UI (`/Identity` routes) that coexist with the Vue SPA. These use Tailwind CSS compiled from `AIInterviewer/tailwind.input.css` to `AIInterviewer/wwwroot/css/app.css`.
...
Configured in [Configure.BackgroundJobs.cs](AIInterviewer/Configure.BackgroundJobs.cs) using `BackgroundsJobFeature`. Jobs are commands that implement `IAsyncCommand<T>`.

## Development Workflow

1. **Start dev servers:** `dotnet watch` (starts both .NET and Vite)
2. **Make backend changes:** Edit C# files in `AIInterviewer.ServiceModel` or `AIInterviewer.ServiceInterface`
3. **Restart .NET Server**
4. **Regenerate DTOs:** `cd AIInterviewer.Client && npm run dtos`
5. **Make frontend changes:** Edit Vue files in `AIInterviewer.Client/src`
6. **Add new CRUD feature:**
   - `npx okai init Feature`
   - Edit `AIInterviewer.ServiceModel/Feature.d.ts`
...
1. Frontend build: `cd AIInterviewer.Client && npm run build`
   - Outputs to `AIInterviewer.Client/dist/`

2. Backend publish: `cd AIInterviewer && dotnet publish`
   - Copies frontend build to `wwwroot/`
   - See publish target in [AIInterviewer.csproj](AIInterviewer/AIInterviewer.csproj) lines 49-60
